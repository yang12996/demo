...
class TestWithdraw(unittest.TestCase):
    ...
    def test_withdraw(self,item):
         ...
         print("体现后的余额" + str(end_amount))

         #断言
         try:
            self.assertEqual(expected["code"],res["code"])
            self.assertEqual(expected["msg"],res["msg"])
            # todo*******************校验数据库中用户余额的变化是否等于体现的金额**************************
            if res["msg"] == "OK":
                # 充值成功,用户余额的变化为充值金额
                self.assertEqual(float(start_amount - end_amount), params["amount"])
                balance=float(start_amount - end_amount)
                print("变化的余额"+str(balance),params["amount"])
            else:
                # 充值失败,用户余额的变化为0
